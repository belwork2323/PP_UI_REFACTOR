...
import LoginFooter from "@ui/components/custom/LoginFooter";
...
import GlobalAlert from "@ui/components/common/GlobalAlertSnackBar";
import ResetPasswordForm from "./components/ResetPasswordForm";
import { useResetPasswordHook } from "@hooks/auth/login/useResetPasswordHook";
import { useLoginHook } from "@hooks/auth/login/useLoginHook";
...
const LoginPage = () => {
  ...
  const resetPassword = useResetPasswordHook({ onBack: handleBackFromReset });
  const loginForm = useLoginHook({ navigate });

  const { open, message, severity, loading, hideAlert } = useAlertStore();

  const mode = useThemeStore((s) => s.mode);
  const t = getLoginTheme(mode);
  const pageIcons = icons.auth.login.page;
  ...
  return (
    <Box sx={t.page}>
      <FullPageLoader open={loading} message={STRINGS.AUTH.AUTHENTICATING} />

      <Box sx={t.toggleButton.wrapper}>
        <IconButton onClick={() => useThemeStore.getState().toggleMode()} sx={t.toggleButton.root}>
          {mode === "dark"
            ? <pageIcons.lightModeToggle sx={t.toggleButton.icon} />
            : <pageIcons.darkModeToggle sx={t.toggleButton.icon} />}
        </IconButton>
      </Box>

      <Box sx={t.belLogo.wrapper}>
        <img
          src={APP_IMAGES.belLogoDark}
          alt="BEL Logo"
        ...
        <Box sx={t.leftPanel.inner}>
          <Box sx={t.drdoLogo.wrapper}>
            <Box sx={t.drdoLogo.badge}>
              <img src={APP_IMAGES.drdoLogo} alt="DRDO Logo" style={t.drdoLogo.img as React.CSSProperties} />
            </Box>
          </Box>
                ...
                <>
                  ...
                  <Box sx={t.form.wrapper}>
                    ...
                    {loginForm.showDeptDropdown && (
                      <Dropdown
                        label={STRINGS.AUTH.SUB_DEPARTMENT_LABEL}
