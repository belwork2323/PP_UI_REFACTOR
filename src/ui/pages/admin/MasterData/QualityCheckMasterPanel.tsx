...
import useMixTypeOptions from "@hooks/admin/MasterData/useMixTypeOptions";
import {
...
import QualityCheckMasterFormDialog from "./QualityCheckMasterFormDialog";
import QualityCheckMasterViewDialog from "./QualityCheckMasterViewDialog";
import QualityCheckMasterTableToolbar from "./components/QualityCheckMasterTableToolbar";
...
const QualityCheckMasterPanel = ({
  ...
  const { options: mixTypeOptions } = useMixTypeOptions();
  ...
  const [viewTarget, setViewTarget] = useState<QualityCheckRecord | null>(null);

  return (
    <Box>
      <Paper elevation={0} sx={table.paper}>
        <QualityCheckMasterTableToolbar
          search={hook.search}
          onSearchChange={hook.setSearch}
          motorStageFilter={hook.motorStageFilter}
          onMotorStageFilterChange={hook.setMotorStageFilter}
          mixTypeFilter={hook.mixTypeFilter}
          onMixTypeFilterChange={hook.setMixTypeFilter}
          motorStageOptions={motorStageOptions}
          mixTypeOptions={mixTypeOptions}
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={table.headerRow}>
                <TableCell sx={table.headerCell}>{S.QUALITY_CHECKS.COL_MIX_TYPE}</TableCell>
                <TableCell sx={table.headerCell}>{S.QUALITY_CHECKS.COL_MOTOR_STAGE}</TableCell>
                <TableCell sx={table.headerCell}>{S.QUALITY_CHECKS.COL_PARAMETERS}</TableCell>
                ...
                hook.paginated.map((row) => (
                  <TableRow key={row.id} sx={table.row}>
                    <TableCell sx={table.cell}>
                      <Typography sx={table.bodyText}>{formatMixTypeLabel(row.mixType)}</Typography>
                    </TableCell>
                    <TableCell sx={table.cell}>
      ...
      <MasterDataToggleConfirmDialog
        target={
          hook.toggleTarget
            ? {
                name:
                  hook.toggleTarget.record.qualityCheckCode ||
                  `${formatMixTypeLabel(hook.toggleTarget.record.mixType)} (stage ${hook.toggleTarget.record.motorStage})`,
