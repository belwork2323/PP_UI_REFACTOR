...
const { CheckCircle, Warning, FiberManualRecord, Error: ErrorIcon, Schedule } = icons.systemManager;

// ── Panel wrapper ─────────────────────────────────────────────────────────────
export const Panel = ({ children, sx = {}, t }) => (
  <Box sx={{ ...t.panel.wrapper, ...sx }}>{children}</Box>
);
    ...
    <Stack direction="row" alignItems="center" gap={0}>
      {labels.map((label, i) => (
        <React.Fragment key={label}>
          ...
          {i < labels.length - 1 && <Box sx={ld.connector(i, active)} />}
        </React.Fragment>
      ))}
    </Stack>
  );
