    ...
    <Box sx={sd.heroCard}>
      ...
      <Box sx={sd.summaryGrid}>
        <Box sx={sd.summaryCard("info")}>
          ...
          <Typography sx={sd.summaryMeta}>
            {viewModel.safeDetails.progressInsights.filledSpecs}/
            {viewModel.safeDetails.progressInsights.totalSpecs || 0}{" "}
            {SDL.FILLED_SPECS.toLowerCase()}
          </Typography>
        </Box>
        <Box sx={sd.summaryCard("passed")}>
          ...
          <Typography sx={sd.summaryMeta}>
            {viewModel.safeDetails.qualityInsights.passRate}% {SDL.PASS_RATE.toLowerCase()}
          </Typography>
        </Box>
        ...
        <Box sx={sd.summaryCard("neutral")}>
          ...
          <Typography sx={sd.summaryMeta}>
            {viewModel.safeDetails.documents[0]?.documentType || NO_DATA_TEXT}
          </Typography>
        </Box>
        <Box sx={sd.summaryCard(viewModel.materialFailures > 0 ? "failed" : "passed")}>
          <Typography sx={sd.summaryValue(viewModel.materialFailures > 0 ? "failed" : "passed")}>
            {viewModel.materialFailures}
          </Typography>
          <Typography sx={sd.summaryLabel}>{SDL.MATERIAL_FAILURES}</Typography>
          <Typography sx={sd.summaryMeta}>
            {viewModel.safeDetails.riskInsights.riskLevel} risk
          </Typography>
        </Box>
      </Box>
    </Box>
        ...
        <Box sx={sd.readinessGrid}>
          <Box sx={sd.readinessItem("passed", viewModel.readiness.readyForApproval)}>
            ...
            <Typography sx={sd.readinessValue("passed")}>
              {viewModel.readiness.readyForApproval
                ? SDL.READY_FOR_APPROVAL
                : SDL.NOT_READY_FOR_APPROVAL}
            </Typography>
          </Box>
          ...
          <Box sx={sd.readinessItem("warn", viewModel.readiness.approvalPending)}>
            ...
            <Typography sx={sd.readinessValue("warn")}>
              {viewModel.readiness.approvalPending
                ? SDL.APPROVAL_PENDING
                : SDL.APPROVAL_NOT_PENDING}
            </Typography>
          </Box>
          <Box
            sx={sd.readinessItem(
              viewModel.readiness.reworkRequired ? "failed" : "passed",
              viewModel.readiness.reworkRequired,
            )}
          >
            <Typography sx={sd.readinessLabel}>{SDL.REWORK_REQUIRED}</Typography>
            <Typography
              sx={sd.readinessValue(viewModel.readiness.reworkRequired ? "failed" : "passed")}
            >
    ...
    <DetailSection title={SDL.PROGRESS_INSIGHTS} empty={!hasProgressData} sd={sd}>
      <Box sx={sd.kpiGrid}>
        <Box sx={sd.kpiBox(sd.colors.info)}>
          ...
          <Typography sx={sd.kpiLabel}>
            {SDL.FILLED_SPECS} / {pi.totalSpecs}
          </Typography>
        </Box>
...
export function QualitySection({ data, sd }: { data: BatchSubDeptDetailsModel; sd: any }) {
  ...
  const hasQualityData =
    qi.totalTests > 0 || qi.passed > 0 || qi.failed > 0 || qi.nonConformances.length > 0;
  return (
...
export function WorkflowRiskSection({ data, sd }: { data: BatchSubDeptDetailsModel; sd: any }) {
  ...
  return (
    <DetailSection title={SDL.WORKFLOW_AND_RISK} empty={!hasWorkflowData} sd={sd}>
      ...
      {wi.resubmissionCount > 0 && (
        <Box sx={sd.flagBoxWithMargin(sd.colors.warn)}>
          <Typography sx={sd.flagText(sd.colors.warn)}>
            Resubmissions: {wi.resubmissionCount}
          </Typography>
        </Box>
      )}
      {di.isDelayed && (
      ...
      {ri.riskFactors.length > 0 && (
        <Box sx={sd.flagList}>
          {ri.riskFactors.map((f: string, i: number) => (
            <Typography key={i} sx={sd.flagListItem(sd.colors.warn)}>
              • {f}
            </Typography>
          ))}
        </Box>
...
export function SummarySection({
  ...
  return (
    <DetailSection
      title={viewModel.summarySectionTitle}
      empty={!data.materialsSummary.length}
      sd={sd}
    >
      <Stack spacing={1}>
          ...
          <Box key={i} sx={sd.matRow(m.blockStatus === "Passed")}>
            <Box sx={sd.matHeaderRow}>
              <Box>
                ...
                <Typography sx={sd.matMeta}>
                  {viewModel.summaryRowMetaPrefix}: {m.lotNo || "—"}
                </Typography>
              </Box>
              <Stack direction="row" alignItems="center" sx={sd.matStatsRow}>
                <Typography sx={sd.infoLabel}>
                  {m.passedSpecs}/{m.totalSpecs} specs
                </Typography>
                <Box component="span" sx={sd.matBadge(m.blockStatus)}>
                  {m.blockStatus}
                </Box>
              </Stack>
            </Box>
            ...
            {m.failures.length > 0 && (
              <Box sx={sd.matFailureWrap}>
                {m.failures.map((f: any, fi: number) => (
                  <Typography key={fi} sx={sd.flagText(sd.colors.failed)}>
                    • {String(f)}
                  </Typography>
                ))}
              </Box>
        ...
        {data.documents.map((doc, i) => (
          <Box key={i} sx={sd.docRow}>
            <Box>
              ...
              <Typography sx={sd.docType}>
                {doc.documentType} • {doc.fileId || "—"}
              </Typography>
            </Box>
...
export default function SubDeptDetails({
  details,
  loading,
  error,
  onRetry,
