...
import { MotorDetailPanel } from "../../user/qualityControl/NDT/components/NDTDetailsContent";
import {
...
type NDTApproverReviewContentProps = {
...
const NDTApproverReviewContent = ({
  detailView,
  loading,
  activeMotorId,
  onActiveMotorChange,
  ...
  const dt = qcTheme.qualityControl.ndt.details;
  ...
  const enabledMotors = useMemo(
    () => motors.filter((motor) => !isNDTMotorApproverTabDisabled(motor.motorSubmissionStatus)),
    [motors],
  );
  ...
  return (
    <Stack spacing={1.25}>
      ...
      <UserWorkflowNavPanel palette={navPalette}>
        <UserWorkflowTabNav
          title={NDT.MOTOR_NAV_TITLE}
          hint={NDT.MOTOR_APPROVER_NAV_HINT}
          tabs={motorNavTabs}
          ...
          isTabDisabled={(_, index) =>
            isNDTMotorApproverTabDisabled(motors[index]?.motorSubmissionStatus)
          }
          tabTooltip={(_, index) =>
            isNDTMotorApproverTabDisabled(motors[index]?.motorSubmissionStatus)
              ? NDT.MOTOR_APPROVER_TAB_DISABLED
              : undefined
          }
        />
      </UserWorkflowNavPanel>

      {activeMotor && !isNDTMotorApproverTabDisabled(activeMotor.motorSubmissionStatus) ? (
        <Box
          sx={{
            borderRadius: 2.5,
            border: `1px solid ${palette.border}`,
        ...
        >
          ...
          >
            <Typography sx={{ fontSize: "0.8rem", fontWeight: 700, color: palette.primary }}>
              {NDT.MOTOR_CARD_TITLE} · {activeMotor.motorId}
            </Typography>
            <PremixStatusChip
          ...
          {canApproveOrReject ? (
            <Stack direction={{ xs: "column", sm: "row" }} gap={1} mb={1.25} justifyContent="flex-end">
              <Button
                ...
                sx={approverTheme.dialog.rejectAction}
              >
                {NDT.MOTOR_APPROVER_REJECT} {activeMotor.motorId}
              </Button>
              <Button
                variant="contained"
                size="small"
                ...
                sx={approverTheme.dialog.approveAction}
              >
                {NDT.MOTOR_APPROVER_APPROVE} {activeMotor.motorId}
              </Button>
            </Stack>
          ) : null}
        ...
        <Typography sx={dt.emptyText}>{NDT.MOTOR_APPROVER_NO_ACTIONABLE}</Typography>
      )}
    </Stack>
  );
};

export default NDTApproverReviewContent;
