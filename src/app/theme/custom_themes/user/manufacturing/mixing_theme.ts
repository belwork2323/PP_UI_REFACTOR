...
export const getMixingTheme = (baseTheme: any) => {
	...
	const accentColor = palette.primaryLight ?? MIXING_BRAND.mxLight;
	...
	return {
		...
		flowBar: {
			container: {
				...
				p: 1.5,
				borderRadius: 3,
				border: `1px solid ${alpha(palette.border ?? MIXING_BRAND.border, 0.9)}`,
				background: alpha(palette.surface ?? MIXING_BRAND.surface, 0.6),
			...
			selectInput: (hasValue: boolean) => ({
				"& .MuiOutlinedInput-root": {
					...
					"& fieldset": {
						borderColor: alpha(palette.border ?? MIXING_BRAND.border, 0.95),
					},
					"&:hover fieldset": {
					...
					"&.Mui-disabled": {
						background: alpha(palette.surface ?? MIXING_BRAND.surface, 0.8),
					},
				},
				"& .MuiSelect-select": {
					fontWeight: hasValue ? 600 : 500,
					color: hasValue ? palette.text ?? MIXING_BRAND.text : palette.textSub ?? MIXING_BRAND.textSub,
					py: 1,
				},
				"& .MuiSelect-icon": {
					color: hasValue ? accentColor : alpha(palette.textSub ?? MIXING_BRAND.textSub, 0.6),
				},
			}),
			selectPlaceholder: {
				color: palette.textSub ?? MIXING_BRAND.textSub,
				fontSize: "0.82rem",
				fontWeight: 500,
			},
			selectMenuPaper: {
				...
				mt: 0.5,
				maxHeight: 320,
				boxShadow: `0 10px 28px ${alpha(MIXING_BRAND.mx, 0.14)}`,
				border: `1px solid ${alpha(palette.border ?? MIXING_BRAND.border, 0.85)}`,
				"& .MuiMenuItem-root": {
					fontSize: "0.82rem",
					py: 1,
					px: 1.5,
			...
			menuItem: (selected: boolean) => ({
				...
				color: selected ? accentColor : palette.text ?? MIXING_BRAND.text,
			...
			motorSelectorBox: {
				p: "14px 18px",
				borderRadius: 3,
				background: `linear-gradient(135deg, ${alpha(MIXING_BRAND.mx, 0.04)}, ${alpha(MIXING_BRAND.mxLight, 0.03)})`,
				border: `1.5px dashed ${alpha(MIXING_BRAND.mxLight, 0.35)}`,
			},
		},
		details: {
			bannerStatusConfig: (() => {
				const primary = palette.primary ?? MIXING_BRAND.primary;
				const primaryLight = palette.primaryLight ?? MIXING_BRAND.primaryLight;
				const success = palette.accent ?? MIXING_BRAND.accent;
				const danger = palette.danger ?? MIXING_BRAND.danger;
				const warnBase = palette.warn ?? MIXING_BRAND.warn;
				return {
			...
			document: {
				...
				boxShadow: `0 4px 24px ${alpha(palette.primary ?? MIXING_BRAND.primary, 0.08)}`,
				overflow: "hidden",
				background: palette.pageBg ?? "#fff",
			},
			banner: {
			...
			section: {
				...
				p: 2,
				borderRadius: 2,
				border: `1px solid ${alpha(palette.border ?? MIXING_BRAND.border, 0.65)}`,
				background: palette.pageBg ?? "#fff",
			},
			sectionTitle: {
				fontSize: "0.72rem",
				...
				color: palette.primaryLight ?? MIXING_BRAND.primaryLight,
				mb: 1.5,
				display: "flex",
				alignItems: "center",
				gap: 0.75,
			...
			metaItem: {
				p: 1.25,
				borderRadius: 1.5,
				background: alpha(palette.primaryLight ?? MIXING_BRAND.primaryLight, 0.04),
				border: `1px solid ${alpha(palette.primaryLight ?? MIXING_BRAND.primaryLight, 0.12)}`,
			},
			metaLabel: { fontSize: "0.62rem", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: palette.textSub ?? MIXING_BRAND.textSub },
			metaValue: { fontSize: "0.88rem", fontWeight: 700, color: palette.text ?? MIXING_BRAND.text, mt: 0.35 },
			blockWrapper: (isLast: boolean) => ({ mb: isLast ? 0 : 2.5 }),
			blockMeta: { fontSize: "0.72rem", color: palette.textSub ?? MIXING_BRAND.textSub },
			blockMetaStrong: { color: palette.text ?? MIXING_BRAND.text, fontWeight: 700 },
			materialChip: {
				height: 22,
				fontSize: "0.68rem",
				fontWeight: 800,
			...
			emptyText: { fontSize: "0.85rem", color: palette.textSub ?? MIXING_BRAND.textSub, textAlign: "center", py: 4 },
