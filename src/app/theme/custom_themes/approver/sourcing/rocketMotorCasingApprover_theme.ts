...
export const getRocketMotorCasingApproverTheme = (mode = "light") => {
  const shared = getSharedTheme(mode);
  const isDark = mode === "dark";

  const palette = {
    primary: isDark ? "#90caf9" : "#1A5276",
    primaryLight: isDark ? "#64b5f6" : "#2980B9",
...
export default getRocketMotorCasingApproverTheme;
