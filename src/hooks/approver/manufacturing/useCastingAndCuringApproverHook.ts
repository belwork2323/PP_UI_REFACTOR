...
export const useCastingAndCuringApproverHook = () => {
  ...
  const [detailView, setDetailView] = useState<CastingCuringDetailView | null>(null);
  ...
  const handleViewDetails = async (row: ApproverListRow) => {
    ...
    setDetailView(
      mapCastingCuringDetailsForDisplay(response.data as unknown as Record<string, unknown>),
    );
  };
