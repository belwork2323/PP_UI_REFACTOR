...
import castingCuringApproverController from "../../../controllers/approver/castingCuringApproverController";
...
import type { CastingCuringApproverMotorChangeStatusPayload } from "../../../data/api/approver/castingCuringApproverApi";
import {
  canApproverActionEntireCastingCuringForm,
  getCastingCuringBatchStatusLabel,
  isCastingCuringMotorApproverTabDisabled,
  mapCastingCuringDetailsForDisplay,
...
const SUB_DEPARTMENT = "casting-and-curing";
...
const resolveInitialApproverMotorId = (
  motors: CastingCuringDetailView["motors"],
): string | null => {
  ...
  const viewableMotor = motors.find(
    (motor) => !isCastingCuringMotorApproverTabDisabled(motor.motorSubmissionStatus),
  );
  return viewableMotor?.motorId ?? null;
};

export const useCastingAndCuringApproverHook = () => {
  ...
  const refreshSelectedDetails = useCallback(
    async (formId: string, preferredMotorIds?: Array<string | number> | null) => {
      ...
      return mapCastingCuringDetailsForDisplay(
  ...
  const submitMotorChangeStatus = useCallback(
    async (payload: Record<string, unknown>) =>
      castingCuringApproverController.submitMotorStatusChange({
        formId: String(payload.formId ?? ""),
        motorId: String(payload.motorId ?? ""),
        subDepartmentId: Number(payload.subDepartmentId ?? 0),
        actionType: payload.actionType as CastingCuringApproverMotorChangeStatusPayload["actionType"],
  ...
  const submitFormChangeStatus = useCallback(
    async (payload: Record<string, unknown>) =>
      castingCuringApproverController.submitFormStatusChange({
        formId: String(payload.formId ?? ""),
        subDepartmentId: Number(payload.subDepartmentId ?? 0),
  ...
  const handleViewDetails = async (row: ApproverListRow) => {
    ...
    const detailView = mapCastingCuringDetailsForDisplay(
      response.data as unknown as Record<string, unknown>,
      {
    ...
    setSelected({
      ...
      status:
        detailView?.status ??
        getCastingCuringBatchStatusLabel((response.data as { status?: unknown })?.status) ??
        row.status,
      detailView,
    });
  };
...
export default useCastingAndCuringApproverHook;
