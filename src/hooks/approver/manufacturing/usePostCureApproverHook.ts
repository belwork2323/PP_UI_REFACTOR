...
import postCureApproverController from "../../../controllers/approver/postCureApproverController";
...
import type { PostCureApproverMotorChangeStatusPayload } from "../../../data/api/approver/postCureApproverApi";
import {
  canApproverActionEntirePostCureForm,
  getPostCureBatchStatusLabel,
...
const resolveInitialApproverMotorId = (
  motors: PostCureDetailView["motors"],
): string | null => {
  ...
  const viewableMotor = motors.find(
    (motor) => !isPostCureMotorApproverTabDisabled(motor.motorSubmissionStatus),
  );
...
export const usePostCureApproverHook = () => {
  ...
  const refreshSelectedDetails = useCallback(async (formId: string) => {
    ...
    return mapPostCureDetailsForDisplay(response.data as unknown as Record<string, unknown>);
  }, []);

  const submitMotorChangeStatus = useCallback(
    async (payload: Record<string, unknown>) =>
      postCureApproverController.submitMotorStatusChange({
        formId: String(payload.formId ?? ""),
        motorId: String(payload.motorId ?? ""),
        subDepartmentId: Number(payload.subDepartmentId ?? 0),
        actionType: payload.actionType as PostCureApproverMotorChangeStatusPayload["actionType"],
  ...
  const submitFormChangeStatus = useCallback(
    async (payload: Record<string, unknown>) =>
      postCureApproverController.submitFormStatusChange({
        formId: String(payload.formId ?? ""),
        subDepartmentId: Number(payload.subDepartmentId ?? 0),
      ...
      setSelected((current) =>
        current
          ? {
              ...
              pcStatus: batchStatus || current.pcStatus,
  ...
  const handleViewDetails = async (row: ApproverListRow) => {
    ...
    const detailView = mapPostCureDetailsForDisplay(
    ...
    const nextStatus =
      detailView?.status ||
      getPostCureBatchStatusLabel((response.data as { status?: unknown; pcStatus?: unknown })?.status) ||
      getPostCureBatchStatusLabel((response.data as { pcStatus?: unknown })?.pcStatus) ||
      row.pcStatus ||
      row.status ||
      "";
    ...
    const enrichedDetailView = detailView
      ? {
          ...
          formSubmissionType:
            detailView.formSubmissionType || (isWaitingForComplete ? "SUBMIT" : detailView.formSubmissionType),
        }
      : null;
    ...
    (item: ApproverListRow) => {
      if (
        !canApproverActionEntirePostCureForm({
          ...
          status: item.detailView?.status ?? item.pcStatus ?? item.status,
