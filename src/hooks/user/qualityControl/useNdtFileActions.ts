...
export type NdtFileAcceptMode = "image" | "imageVideo" | "pdf";
export type NdtSubDeptSlug = "ndt" | "qc-division";

const validateNdtUploadFile = (
  file: File,
  mode: NdtFileAcceptMode,
...
/** Eager upload / retry / remove / open for NDT `CasePrepFileRef` lists. */
export function useNdtFileActions(
  files: CasePrepFileRef[],
  onChange: (next: CasePrepFileRef[]) => void,
  options?: { acceptMode?: NdtFileAcceptMode; subDeptSlug?: NdtSubDeptSlug },
) {
  const acceptMode = options?.acceptMode ?? "imageVideo";
  const subDeptSlug = options?.subDeptSlug ?? "ndt";
  ...
  const handleFilesSelected = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      for (const file of incoming) {
        const { valid, error } = validateNdtUploadFile(file, acceptMode);
        if (!valid) {
