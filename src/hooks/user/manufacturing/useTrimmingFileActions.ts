...
export type TrimmingFileAcceptMode = "imageVideo" | "imageVideoPdf";

const validateTrimmingUploadFile = (
  file: File,
  mode: TrimmingFileAcceptMode,
...
/** Eager upload / retry / remove / open for trimming report `CasePrepFileRef` lists. */
export function useTrimmingFileActions(
  files: CasePrepFileRef[],
  onChange: (next: CasePrepFileRef[]) => void,
  options?: { acceptMode?: TrimmingFileAcceptMode },
) {
  ...
  const handleFilesSelected = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      for (const file of incoming) {
        const { valid, error } = validateTrimmingUploadFile(file, acceptMode);
        if (!valid) {
