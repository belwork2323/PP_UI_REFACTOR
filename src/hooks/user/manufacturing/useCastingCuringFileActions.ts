...
export type CastingCuringFileAcceptMode = "imageVideo" | "imageVideoPdf";

const validateCastingCuringUploadFile = (
  file: File,
  mode: CastingCuringFileAcceptMode,
...
/** Eager upload / retry / remove / open for Casting & Curing `CasePrepFileRef` lists (RMS/RMC parity). */
export function useCastingCuringFileActions(
  files: CasePrepFileRef[],
  onChange: (next: CasePrepFileRef[]) => void,
  options?: { acceptMode?: CastingCuringFileAcceptMode },
) {
  ...
  const handleFilesSelected = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      for (const file of incoming) {
        const { valid, error } = validateCastingCuringUploadFile(file, acceptMode);
        if (!valid) {
