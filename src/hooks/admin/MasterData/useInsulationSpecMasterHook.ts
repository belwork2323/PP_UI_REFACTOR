...
import {
  buildInsulationCreatePayload,
  buildInsulationDeletePayload,
  buildInsulationUpdatePayload,
  createEmptyInsulationForm,
  emptyMasterDataStats,
  InsulationSpecListModel,
  mapInsulationRecordToForm,
  validateInsulationForm,
  type InsulationSpecFormState,
  type InsulationSpecListPayload,
  type InsulationSpecRecord,
...
export default function useInsulationSpecMasterHook({
  activeFilter,
  refreshKey = 0,
  onListPayloadChange,
  onStatsChange,
  ...
  const [items, setItems] = useState<InsulationSpecRecord[]>([]);
  const [listPayload, setListPayload] = useState<InsulationSpecListPayload | null>(null);
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      const resp = new ApiResponseModel(await fetchInsulationSpecMasterList(body), InsulationSpecListModel.fromApi);
  ...
  const saveForm = async () => {
    ...
    try {
      const raw = isEdit
        ? await updateInsulationSpecMaster(buildInsulationUpdatePayload(form))
        : await createInsulationSpecMaster(buildInsulationCreatePayload(form));
